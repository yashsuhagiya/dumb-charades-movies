...
export function MovieCard({ movie, index }: { movie: Movie; index: number }) {
  return (
    ...
    >
      ...
      <div className="mt-6 flex flex-wrap gap-x-3 gap-y-1 font-mono text-[10px] uppercase tracking-[0.18em] text-cream/75">
        <span>{movie.industry}</span>
        <span className="text-dust">·</span>
        <span>{movie.language}</span>
        {movie.genres.slice(0, 2).map((g) => (
          <span key={g}><span className="text-dust">· </span>{g}</span>
        ))}
      </div>

      {movie.hint && (
        <p className="mt-4 italic text-xs text-cream/80 font-mono">
